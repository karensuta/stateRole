        ...
        <tr>
          <td>
            ...
            <?php
              ...
              $sec=$cxn->query("SELECT * FROM formacion INNER JOIN tipo_programa ON formacion.id_tipo_programa=tipo_programa.id_tipo_programa WHERE estado=1");

              while ($row=$sec->fetch(PDO::FETCH_ASSOC)) {
                echo "<option value='$row[id_formacion]'>$row[tipo_programa] - $row[programa]</option>";
              }
            ?>
          </select>
        </tr>
